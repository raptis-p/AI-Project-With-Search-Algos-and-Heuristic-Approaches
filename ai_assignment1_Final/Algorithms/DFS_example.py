...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    def heuristic_function(self, node_current):
        ...
        node_center = self.get_node_information(node_current)
        goal_center = self.get_goal_information()
        # sign = -1 if (node_center[0] - goal_center[0] > 0) else 1
        distance = math.sqrt(math.pow(node_center[0] - goal_center[0], 2) + math.pow(node_center[1] - goal_center[1], 2))
        return distance
    ...
    def recursive_DFS(self, node_current):
        ...
        for primitive_successor in node_current.get_successors():
            ...
            # print("Node position is: ", child.get_position())
            # print("And path to get here is: ", child.get_path())
    ...
    def execute_search(self,w, time_pause):
