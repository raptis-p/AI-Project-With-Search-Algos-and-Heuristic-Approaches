...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    def update_visuals(self):
        ...
        """
        position = self.path_new[-1][-1].position.tolist()
        # print("In update: ", position, "with type: ", type(position))
        # if len(self.visited_nodes) > 0:
        #     print("My current list is: ", type(self.visited_nodes[0]))
    ...
    def heuristic_function2(self, node_current):
        ...
        distance = abs(Xcurr-Xgoal) + abs(Ycurr-Ygoal)
    ...
    def heuristic_function3(self, node_current):
        ...
        if(sum==obst.__len__()):
            ...
            distance =  2*math.sqrt( (a**2+(c ** 2 + a ** 2)))
    ...
    def initializeIDAstar(self,limit,node_initial, w,heuristic,finalPath):
        #print("Initialize ida* with limit:", limit)
        #node_initial = self.initialize_search(time_pause=time_pause)
        fringe = []
        fringe.append(node_initial)
        result,finalPath = self.IDAstarSearch(node_initial, fringe, limit, node_initial, w,heuristic,[])
    ...
    def IDAstarSearch(self,current_node,fringe,limit,node_initial, w,heuristic,finalPath):
        if (limit<self.evaluation_function(current_node,w,heuristic)):

            limit=self.evaluation_function(current_node,w,heuristic)
            self.initializeIDAstar(limit,node_initial,w,heuristic,[])
                ...
                #fringe.sort(key=lambda x : self.evaluation_function(x,w,heuristic))
                bestChild = self.returnBestChild(fringe, w, heuristic)
                result = self.IDAstarSearch(bestChild, fringe, limit,node_initial, w,heuristic,finalPath)
    ...
    def execute_search(self,w,heuristic,finalPath, time_pause) -> Tuple[Union[None, List[List[State]]], Union[None, List[MotionPrimitive]], Any]:
        ...
        limit=self.evaluation_function(node_initial, w,heuristic)
        fringe = []
        fringe.append(node_initial)
        result,finalPath = self.IDAstarSearch(node_initial, fringe,limit, node_initial, w,heuristic,[])
